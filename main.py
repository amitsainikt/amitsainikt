...
def Crunchy_Roll(message):
    result = bot.get_chat_member(CHAT_ID1, message.chat.id)
    if str(result.status) == 'member':
        limiting = 'https://tempbane.pythonanywhere.com/limit?uid='+str(message.chat.id)
...
def clearing_limit(message):
    res = requests.get('https://tempbane.pythonanywhere.com/clear')
    bot.send_message(message.chat.id, "Cleared everyones limit")


@bot.message_handler(commands=['status'])
def checking_gen_status(message):
    status_check = "https://tempbane.pythonanywhere.com/status?uid="+str(message.chat.id)
...
def Paytm(message):
    ...
    if str(result.status) == 'member':
        ...
        if 'Success' not in reslimit.text:
            ...
            res = requests.get('https://ghostbin.com/WyOrr/raw')
